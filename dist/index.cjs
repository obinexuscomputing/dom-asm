...
const packageJsonPath = path$1.resolve(__dirname, "../package.json");
    ...
    try {
        switch (type) {
            case "css":
                ...
                if (options.validate) {
                    const validator = new css.CSSValidator(ast);
                ...
                if (options.optimize) {
                    const optimizer = new css.CSSASTOptimizer(ast);
                    ast = optimizer.optimize();
                    const generator = new css.CSSCodeGenerator(ast);
            ...
            case "html":
                ...
                if (options.validate) {
                    const validator = new html.HTMLValidator();
                ...
                if (options.optimize) {
                    const optimizer = new html.HTMLASTOptimizer();
                    ast = optimizer.optimize(ast);
                    const generator = new html.HTMLCodeGenerator();
            ...
            case "js":
                ...
                if (options.validate) {
                    const validator = new js.JSValidator();
                ...
                if (options.optimize) {
                    const minimizer = new js.JSAstMinimizer();
                    ast = minimizer.optimize(ast);
                    const generator = new js.JSCodeGenerator();
            ...
            case "xml":
                ...
                if (options.validate) {
                    const validator = new xml.DOMXMLValidator();
                    const validationResult = validator.validate(ast);
                    if (!validationResult.valid) {
                ...
                if (options.optimize) {
                    const optimizer = new xml.DOMXMLASTOptimizer();
                    ast = optimizer.optimize(ast);
                    const generator = new xml.DOMXMLGenerator();
                    result.optimized = generator.generate(ast);
                }
                break;
            case "asm":
        ...
        if (options.output) {
            fs$1.writeFileSync(options.output, typeof output === "string" ? output : JSON.stringify(output, null, 2));
        }
        else {
            console.log(output);
        }
